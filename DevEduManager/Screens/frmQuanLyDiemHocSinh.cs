...
{
    public partial class frmQuanLyDiemHocSinh : Form
    {
        ...
        public frmQuanLyDiemHocSinh()
        {
            InitializeComponent();
        }

        private async void frmQuanLyDiemHocSinh_Load(object sender, EventArgs e)
        {
            await LoadComboBoxCourseAsync();
        }

        /// <summary>
        /// Load danh sách Chương trình học (Course)
        /// </summary>
        private async Task LoadComboBoxCourseAsync()
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    cboCT.SelectedIndex = -1; // Không chọn ngay
                }
                else
                {
        ...
        /// <summary>
        /// Khi chọn môn học → Load danh sách học viên + điểm vào DataGridView
        /// </summary>
        ...
        /// <summary>
        /// Lấy danh sách học viên và điểm tương ứng
        /// </summary>
            ...
            try
            {
                ...
                if (dtStudents != null && dtStudents.Rows.Count > 0)
                {
                    foreach (DataRow student in dtStudents.Rows)
                    {
                        ...
                        dtFinal.Rows.Add(studentID, fullName, score, status);
                    }
                }

                dtgvDshv.AutoGenerateColumns = false;
                dtgvDshv.DataSource = dtFinal;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi load dữ liệu học viên & điểm: {ex.Message}");
            }
        }

        private void btnNhap_Click(object sender, EventArgs e)
        {
            frmNhapDiemMoi frm = new frmNhapDiemMoi();
            frm.ShowDialog();
